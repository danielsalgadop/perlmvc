...
# Hosts destino #
#################
array_hosts_destino=( "VMAD04TESTSAD1" )
...
function sincronizar(){
	...
	rsync -avz --delete $path_origen --exclude=".git" --exclude="deploy*" --exclude="docs" --exclude="utils" --exclude="prueba*" op1@$host_destino:$path_destino/$nombre_app_destino
	if [ ! $? -eq 0 ]; then
